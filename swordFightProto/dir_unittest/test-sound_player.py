...
### MusicPlayer() ###
    def test_loadSong(self):
        self.assertEqual(self.musicPlayer.loadSong(self.musicPath, 'testsong', '.mp3'), True)
        self.assertEqual(self.musicPlayer.loadSong(self.musicPath, 'songnotfound', '.mp3'), False)
        self.assertEqual(self.musicPlayer.loadSong(self.musicPath, 'testsong', '.mp3'), False) #already loaded
...
### SoundEffectPlayer() ###
    def test_loadSoound(self):
        self.assertEqual(self.soundEffectPlayer.loadSound(self.effectPath, 'testeffect', '.wav'), True)
        self.assertEqual(self.soundEffectPlayer.loadSound(self.effectPath, 'effectnotfound', '.wav'), False)
        self.assertEqual(self.soundEffectPlayer.loadSound(self.effectPath, 'testeffect', '.wav'), False) #already loaded
